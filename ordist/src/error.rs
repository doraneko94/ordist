...
impl<K: OrDistElement> fmt::Display for OrDistError<K> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            OrDistError::ElementNotFound(e) => write!(f, "Element not found!: element={:?}", e),
            OrDistError::RankOutOfLength { length, rank, elem } => write!(
                f,
                "Rank out of length!: length={}, rank={}, element={:?}",
                length, rank, elem
            ),
            OrDistError::DuplicateRank(rank) => write!(f, "Duplicate rank!: rank={}", rank),
        }
    }
}
...
impl<K: OrDistElement> From<OrDistError<&K>> for OrDistError<K> {
    fn from(error: OrDistError<&K>) -> Self {
        match error {
            ...
            OrDistError::ElementNotFound(e) => OrDistError::ElementNotFound(e.clone()),
            OrDistError::RankOutOfLength { length, rank, elem } => OrDistError::RankOutOfLength {
                length,
                rank,
                elem: elem.clone(),
            },
            OrDistError::DuplicateRank(rank) => OrDistError::DuplicateRank(rank),
        }
    }
}
